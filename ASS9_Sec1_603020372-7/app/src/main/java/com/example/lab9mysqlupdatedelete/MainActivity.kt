...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recycler_view.addOnItemTouchListener(object : OnItemClickListener {
            override fun onItemClicked(position: Int, view: View){
                Toast.makeText(applicationContext, "You click on : "+studentList[position].std_id, Toast.LENGTH_SHORT).show()
    ...
    fun clickSearch(v: View){
        studentList.clear()
        createClient.retrieveStudentID(edt_search.text.toString()).enqueue(object:
            Callback<Student>{
